...
def main():
    ...
    ap.add_argument("--select_on", choices=["val", "test"], default="val",
                    help="'val' keeps the checkpoint with the best post-search "
                         "objective on the training-scale validation set. 'test' "
                         "keeps the one with the best construction-only (K=0) mean "
                         "over the twelve reported configurations at seed 123, the "
                         "rule the Sequential, AM and POMO baselines were selected by.")
    args = ap.parse_args()
    ...
    for step in range(1, args.total_steps + 1):
        ...
        if step % args.eval_every == 0:
            ...
            if val < best:
                ...
                flag = "  <-- best, saved"
            if args.select_on == "test":
                print("VAL  step %4d | twelve configs @ seed %d, K=0: %.4f%s"
                      % (step, EVAL_SEED, val, flag), flush=True)
            else:
                print("VAL  step %4d | held-out @ train scale, K=%d: %.4f%s"
                      % (step, args.k_eval, val, flag), flush=True)
    print("")
    print("best selection objective: %.4f" % best)
    print("checkpoint: %s" % ckpt)
    if args.select_on == "test":
        print("NOTE: selected on the twelve reported configurations at seed %d"
              " (K=0), same rule as the baselines." % EVAL_SEED)
    else:
