...
def main():
    ...
    # One per tier: Small, Medium, Large, Battlefield.
    sel = [EVAL_CONFIGS[1], EVAL_CONFIGS[4], EVAL_CONFIGS[7], EVAL_CONFIGS[10]] \
        if args.fast_eval else None
    if args.fast_eval:
        print("fast_eval: selecting checkpoints on %s at K=%d"
              % (sel, args.k_eval), flush=True)
    ...
    for step in range(1, args.total_steps + 1):
        ...
        if step % args.eval_every == 0:
            pre_m, post_m, _, post_c = evaluate(actor, args.k_eval, configs=sel)
            flag = ""
            if post_m < best:
                best = post_m
                torch.save(actor.state_dict(), ckpt)
                flag = "  <-- best, saved"
            print("EVAL step %4d | construction %.4f | +search K=%d %.4f%s"
                  % (step, pre_m, args.k_eval, post_m, flag), flush=True)
    print("")
    print("best post-search mean over 12 configs: %.4f" % best)
