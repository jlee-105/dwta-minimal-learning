...
the other methods that learn a policy: AM, POMO and Sequential. Greedy and
Auction are fixed mechanisms with no parameters to select or train, and SCIP
is an exact solver, so neither pairing is informative in the same way; SCIP's
comparison is the tier split given above (Appendix~\ref{app:scip}). Our
per-instance value, averaged over the ten seeds, is compared against each
learned baseline's value on the same 120 seed-123 instances with a paired
two-sided Wilcoxon signed-rank test. Against AM and POMO the advantage is
significant overall ($p<10^{-4}$ for both) and at every configuration but one
(POMO ties at (50,50,15) and (70,100,15), consistent with POMO's edge there).
Against Sequential the instance-level record is 79 wins and 41 losses of 120,
significant overall ($p=0.0004$); at the configuration level, only two of the
four Large-tier losses survive a Holm correction across the twelve
configurations, (30,40,10) and (40,50,10), and the (15,15,5) tie is confirmed
($p=0.70$). All tests and per-instance values are in Appendix~\ref{app:paired}.
