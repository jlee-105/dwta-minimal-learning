...
configurations in each tier. All methods decode one instance at a time on the
same hardware. SCIP is omitted: it is given a fixed 600\,s budget per instance
rather than running to completion, so its time is a setting, not a
measurement.}
\label{tab:timing}
