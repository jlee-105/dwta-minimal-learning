...
its own raw output, reduced the mean objective by 10.7\% relative to the
...
between 0.003 and 0.025, and POMO at one, by 0.010. The mean advantage is
carried by the larger configurations rather than spread evenly. Training
exhibits the early-peak-then-oscillate behaviour common to every method we
trained on this problem, so the reported checkpoint is selected by best
validation performance during training rather than final step; selection uses
instances disjoint from those reported, and we report the full evaluation
trace rather than the peak alone, but it remains a
