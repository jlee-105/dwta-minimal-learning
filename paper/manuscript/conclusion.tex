...
its own raw output, reduced the mean objective by 13.1\% relative to the
...
Several limits bound these claims. Results are reported over ten seeds, with
across-seed standard deviation between 0.001 and 0.028, and the advantage is
not uniform: a sequential decoder edges ahead at four configurations, by
between 0.002 and 0.022, three of them in the Large tier, and POMO at one, by
0.008. Training exhibits the early-peak-then-oscillate behaviour common to
every method we trained on this problem, so the reported checkpoint is
selected by best objective during training rather than final step. Every
learned method is selected by the same rule on the reported configurations,
and we report the full evaluation trace rather than the peak alone, but it
remains a selection procedure that a single-run reader cannot verify. Finally, the local search's operator is a single participation flip.
